...
public class MIABlockListener extends BlockListener {
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
    	...
    	if ((town > 0 && town != plugin.mf.playertownId(event.getPlayer()) || (plugin.mf.inzoneProp(event.getBlock(), "protect") == 1 && !plugin.mf.ownzone(event.getPlayer()))) && !event.getPlayer().isOp()) {
    ...
    public void onBlockInteract(BlockInteractEvent event) {
    	if ((event.getBlock().getType() == Material.CHEST || event.getBlock().getType() == Material.BURNING_FURNACE || event.getBlock().getType() == Material.FURNACE || event.getBlock().getType() == Material.WORKBENCH) && ((!plugin.mf.inowntown((Player) event.getEntity()) && plugin.mf.intown((Player) event.getEntity()) > 0) || (plugin.mf.inzoneProp(event.getEntity().getLocation(), "chest") == 1 && !plugin.mf.ownzone((Player) event.getEntity())))) {
